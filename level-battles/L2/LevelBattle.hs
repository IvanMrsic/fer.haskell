...
lb22 :: Eq a => a -> [a] -> Int
lb22 y xs = matcher y xs 0 
  where matcher y [] n = -1
        matcher y (x:xs) n | x == y = n
                           | x /= y = matcher y xs (n+1)
...
lb24 []  = []
lb24 [x] = [[x]]
lb24 xs = xs:(lb24 $ triangle xs)
  where triangle [x]      = [x]
        triangle [x, y]   = [x+y]
        triangle (x:y:xs) = x+y : triangle (y:xs)
