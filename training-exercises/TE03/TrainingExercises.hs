...
te311 (_, 'H':x:y:z:w:u:t:_, _) = True
te311 _                         = False
...
te321 :: [Double] -> Double
te321 xs = average middle
  where middle     = tail (init xs)
        average [] = error "Can't average over empty list"
...
te322 :: String -> String
te322 xs = case strlen of
  0 -> error "Empty list is not a string"
  1 -> "The string has one character"
  2 -> "The string has two characters"
  _ -> "The string has many characters"
...
te323 [] = error "te323: Empty list"
te323 xs = (median m1, median xs, median m2)
  where m1 = fst $ splitTwo srt
        m2 = tail $ snd $ splitTwo srt
        srt = sort xs 
        
splitTwo :: (Integral a) => [a] -> ([a], [a])
splitTwo xs = splitAt h xs
  where l = length xs
        h = l `div` 2
