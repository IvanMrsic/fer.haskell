...
te915 xs f1 f2 = map(\(Pair a b) -> Pair (f1 a) (f2 b)) xs
...
te922 xs = Nothing `notElem` xs && (Just False) `notElem` xs
...
mapGrade Nothing      = Nothing
mapGrade (Just True)  = (Just 5)
mapGrade (Just False) = (Just 1)

te931 :: [Maybe Bool] -> [Maybe Int]
te931 = map (\x -> mapGrade x)
...
te943 = treeMax . treeToList
  where treeMax [] = Nothing
        treeMax xs = Just (maximum xs)
