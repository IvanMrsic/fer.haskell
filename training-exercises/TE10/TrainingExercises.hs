...
instance Ord a => Eq (MyList a) where
  m1 == m2 = sort (myListToList m1) == sort (myListToList m2)
...
treeToList Ornament             = []
treeToList (Light x left right) = treeToList left ++ [x] ++ treeToList right
...
instance Ord a => Eq (ChristmasTree a) where
  c1 == c2 = sort (treeToList c1) == sort (treeToList c2)