...
getForename :: FruitStock -> Maybe (Maybe String)
getForename = fmap farmerId . farmer

te1211 :: String -> Int
te1211 id = foldl (\s f -> if (getForename f == (Just $ Just $ id)) then s + (amount f) else s) 0 company_stock  

te1211m :: String -> Int
te1211m id = foldl (\s f -> if (farmer f >>= farmerId) == (Just $ id) then s + (amount f) else s) 0 company_stock  
...
getIds :: Fruit -> [String]
getIds f = map (\(Just x) -> x) $ filter (\x -> x /= Nothing) $ map(\x -> farmer x >>= farmerId) $ filter (\x -> f == fruit x) company_stock

te1212 :: IO ()
te1212 = do 
  input <- getLine
  let fr = (read input :: Fruit)
  putStrLn $ show (getIds fr)
...
te1225 :: [Int] -> [Int]
te1225 = (>>= (\x -> if odd x then replicate 3 x else [x]))